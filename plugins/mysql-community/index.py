...
def getBackupDir():
    bk_path = mw.getBackupDir() + "/database/mysql-community"
...
def initDreplace(version=''):
    ...
    systemService = systemDir + '/mysql-community.service'
...
def process_status():
    cmd = "ps -ef|grep mysql-community|grep mysql |grep -v grep | grep -v python | awk '{print $2}'"
...
def importDbExternalProgress():
    ...
    cmd += 'python3 '+mw.getServerDir()+'/mdserver-web/plugins/mysql-community/index.py import_db_external_progress_bar  {"file":"'+file+'","name":"'+name+'"}'
...
def importDbExternalProgressBar():
    ...
    my_cnf = getConf()
    mysql_cmd = getServerDir() + '/bin/mysql --defaults-file=' + my_cnf + ' -uroot -p"' + pwd + '" -f ' + name
...
def syncDatabaseRepairLog(version=''):
    ...
    cmd = 'cd '+mw.getServerDir()+'/mdserver-web && . bin/activate && python3 plugins/mysql-community/index.py sync_database_repair  {"db":"'+sync_args_db+'","sign":"'+sync_args_sign+'"}'
    # print(cmd)
...
def asyncTmpfile():
    path = '/tmp/mysql_community_async_status.txt'
    return path
...
def fullSyncCmd():
    ...
    cmd = 'cd '+mw.getServerDir()+'/mdserver-web && . bin/activate && python3 plugins/mysql-community/index.py do_full_sync  {"db":"'+db+'","sign":"'+sign+'"}'
    return mw.returnJson(True,'ok',cmd)
